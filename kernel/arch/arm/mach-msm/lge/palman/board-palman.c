...
static bool msm_hsusb_vbus_power(bool on)
{
	...
	if (!ext_5v_reg) {
		...
		if (IS_ERR(ext_5v_reg)) {
			...
			return PTR_ERR(ext_5v_reg);
		}
	}

	if (on) {
		rc = regulator_enable(ext_5v_reg);
		if (rc) {
			pr_err("%s : Unable to enable the regulator:"
					" ext_5v_reg\n", __func__);
			return -ENODEV;
		}
		rc = regulator_enable(votg_5v_switch);
		if (rc) {
			pr_err("%s : Unable to enable the regulator:"
					" votg_5v_switch\n", __func__);
			return -ENODEV;
		...
		rc = regulator_disable(votg_5v_switch);
		if (rc) {
			pr_err("%s: Unable to disable the regulator:"
				" votg_5v_switch\n", __func__);
			return -ENODEV;
		}
		rc = regulator_disable(ext_5v_reg);
		if (rc) {
			pr_err("%s: Unable to disable the regulator:"
				" ext_5v_reg\n", __func__);
			return -ENODEV;
