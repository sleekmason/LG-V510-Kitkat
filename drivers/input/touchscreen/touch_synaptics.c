...
#define MANUFACTURER_ID_REG (ts->common_dsc.query_base)    /*                 */
#define FW_REVISION_REG     (ts->common_dsc.query_base+3)  /*             */
#define PRODUCT_ID_REG      (ts->common_dsc.query_base+11) /*            */
#if defined(CONFIG_TOUCH_REG_MAP_TM2000) || defined(CONFIG_TOUCH_REG_MAP_TM2372)
#define FW_VERSION          (ts->flash_dsc.control_base)
#endif

#define DEVICE_CONTROL_REG (ts->common_dsc.control_base)  /*                */
...
#define INTERRUPT_ENABLE_REG (ts->common_dsc.control_base+1) /*                    */

#define DEVICE_STATUS_REG    (ts->common_dsc.data_base)    /*               */
...
#define INTERRUPT_STATUS_REG  (ts->common_dsc.data_base+1) /*                  */
#define BUTTON_DATA_REG       (ts->button_dsc.data_base)   /*             */
#define MAX_NUM_OF_BUTTON 4

#define FINGER_STATE_REG      (ts->finger_dsc.data_base)   /*              */
#define FINGER_DATA_REG_START (ts->finger_dsc.data_base+3) /*                      */
...
#define TWO_D_REPORTING_MODE (ts->finger_dsc.control_base+0) /*                   */
#define REPORT_MODE_CONTINUOUS      0x00
#define REPORT_MODE_REDUCED         0x01
#define ABS_FILTER                  0x08
#define PALM_DETECT_REG      (ts->finger_dsc.control_base+1) /*             */
#define DELTA_X_THRESH_REG  (ts->finger_dsc.control_base+2) /*                */
#define DELTA_Y_THRESH_REG   (ts->finger_dsc.control_base+3) /*                */
#define SENSOR_MAX_X_POS     (ts->finger_dsc.control_base+6) /*               */
#define SENSOR_MAX_Y_POS     (ts->finger_dsc.control_base+8) /*               */
#if defined(CONFIG_TOUCH_REG_MAP_TM2000) || defined(CONFIG_TOUCH_REG_MAP_TM2372)
/*          */
#else
#define GESTURE_ENABLE_1_REG (ts->finger_dsc.control_base+10) /*                   */
#define GESTURE_ENABLE_2_REG (ts->finger_dsc.control_base+11) /*                   */
#endif
